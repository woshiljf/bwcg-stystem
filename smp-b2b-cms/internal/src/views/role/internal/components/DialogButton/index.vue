  ...
  <div class="inline-block">
    ...
    <el-dialog
        ...
        <el-form ref="DialogRuleForm" :model="params" :label-width="`${labelWidth}px`" label-position="left">
          <el-row :gutter="20">
            ...
            >
              <el-col v-if="option.type==='text'" :span="option.span || 20" class="search-bar-col">
                ...
                >
                  <ul v-if="Array.isArray(params[option.name])" class="li-style-none" >
                    <li v-for="(item, index) in params[option.name]" :key="index">{{ params[option.name][index]['remark'] || params[option.name][index]['userName'] }}</li>
                  </ul>
              ...
              <el-col v-if="option.type==='tree'" :span="option.span || 24" class="search-bar-col" >
                <el-form-item
                  :label="option.label"
                >
                  <el-tree :data="params[option.name]" :props="option.defaultProps"/>
                </el-form-item>
              </el-col>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="CancelFuction">{{ cancelText }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created() {
    this.options.forEach((item) => {
      if ((item.type === 'lazySelect' && item.multiple) || (item.type === 'select' && item.multiple) || item.type === 'transfer' || item.type === 'tree') {
