<template>
  <div>
    <!-- 采购单新增页自动分货弹框组件 -->
    <div class="auto-btn">
      <el-button :disabled="!form.inExternalWarehouseCode" type="primary" size="mini" @click="autoDelivery(form.inExternalWarehouseCode)">自动分货</el-button>
    </div>
    ...
    <el-dialog :visible.sync="dialogTableVisible" :close-on-click-modal="false" title="自动分货" width="40%" center>
      ...
      <el-row class="rules">
        <el-form-item>
          ...
          <el-row v-for="(item, index) in allocateList" :key="index">
            <el-col :span="24">
              <el-col :span="5">
    ...
    <el-dialog :visible.sync="hasSetRulesTip" :close-on-click-modal="false" title="自动分货已设置提醒" width="40%" center>
      ...
      <el-row class="rules">
        <el-form-item>
          ...
          <el-row v-for="(item, index) in autoDeliveryInfo" :key="index">
            <el-col :span="24">
              <el-col :span="5">
      ...
      <div class="warehouse-btn">
        <el-button type="primary" size="mini" @click="resetRules(form.inExternalWarehouseCode)">是的，需要重新设置</el-button>
...
export default {
  ...
  methods: {
    ...
    async getNameAndChannel() {
      this.inWarehouseCode = this.form.inExternalWarehouseCode
      this.options4.forEach(item => {
        console.log(item, '调入仓')
        if (item.value === this.form.inExternalWarehouseCode) {
    ...
    async changeWarehouse() {
      if (this.autoDeliveryInfo.length > 0 && this.autoDeliveryInfo.indexOf(this.form.inExternalWarehouseCode) < 0) {
    ...
    async confirm() {
      ...
      this.$emit('childFn', this.autoDeliveryInfo)
      this.$alert('设置成功', '', {
