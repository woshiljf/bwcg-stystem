<template>
  <el-tooltip :disabled="value.length === 0" class="item" effect="light" placement="top">
    ...
    <el-select
      ...
      <el-option :disabled="true" value="选项0" style="width:100%">
      ...
      <el-option
        ...
        <!-- 改为传仓库内码 -->
        <el-tooltip :disabled="item.externalWarehouseCode? item.externalWarehouseCode.length < 10 : true" :content="item.externalWarehouseCode" class="item" effect="light" placement="top">
          <span class="spanClass" >{{ item.externalWarehouseCode }}</span>
        </el-tooltip>
        <!-- 修改到此 -->
...
export default {
  ...
  async created() {
    ...
    this.optionsWarehouse = resWarehouse.data.data.warehouseList.map(item => {
      ...
      return { externalWarehouseCode: item.externalWarehouseCode, value: item.warehouseCode, label: item.warehouseName, type: item.warehouseTypeName, count: item.warehouseAccount, isSelect: false }
    })
  },
  mounted() {
  ...
  methods: {
    ...
    changeList(val) {
      ...
      if (val.length === 0) {
        const str = this.porpsType === '' ? this.warehouseType : this.porpsType
        this.getList('', str)
      }
    },
    delSelect(val) {
