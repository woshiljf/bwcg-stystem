<template>
  <div class="ChannelAllocationBBC">
    <el-container class="stock-box">
      ...
      <div class="pageBar">
        <page-common
          ref="page"
          api="/bff/api/v1/channel_inventory/_page?bizType=BBC"
          method="post"
          @query="query"
          @loading="load"
        />
...
import ChannelBatch from './channelBatchBBC'
...
export default{
  ...
  async created() {
    if (sessionStorage.getItem('urlName') !== '库存管理-渠道库存管理BBC页面') {
      window._paq.push(['setDocumentTitle', '库存管理-渠道库存管理BBC页面'])
      window._paq.push(['trackPageView'])
      sessionStorage.setItem('urlName', '库存管理-渠道库存管理BBC页面')
    }
  ...
  methods: {
    ...
    async searchInventoryLog() {
      ...
      this.$axios.post('/bff/api/v1/inventory_log/_page?bizType=BBC', request).then(res => {
    ...
    async reduceGoods(form) {
      ...
      // 请求500异常统一处理
      const res = await this.$axios.post('/bff/api/v1/channel_inventory/down_info?bizType=BBC', params)
    ...
    Export(params) {
      ...
      this.$axios({
        method: 'post',
        url: `/bff/api/v1/channel_inventory/_export?bizType=BBC`,
        data: form
        // responseType: 'blob'
      }).then(res => {
