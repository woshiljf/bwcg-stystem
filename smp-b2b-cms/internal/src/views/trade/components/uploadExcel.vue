<template>
  <div>
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        <el-upload
          ref="uploade"
          :on-preview="handlePreview"
          :on-remove="handleRemove"
          :before-upload="beforeUpload"
          :limit="1"
          :on-exceed="handleExceed"
          :auto-upload="false"
          class="upload-demo"
          list-type="text"
          accept=".xls,.xlsx,.xlsm"
          action="https://jsonplaceholder.typicode.com/posts/"
          multiple>
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip">一次只能上传一个excel文件，仅限格式xlsx、xls、xlsm，单个文件不超过10MB</div>
        </el-upload>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">{{ $t('table.cancel') }}</el-button>
        <el-button type="primary" @click="updateData()">{{ $t('table.confirm') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script>
import { shipment, sending, reUploadOrder } from '@/api/trade'
export default {
  name: 'UploadExcel',
  props: {
    orderNo: {
      type: String,
      default: ''
    },
    userCode: {
      type: String,
      default: ''
    },
    step: {
      type: String,
      default: ''
    },
    dialogFormVisible: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      temp: {},
      textMap: {
        update: 'Edit',
        packing: '出货单号',
        shipment: '装箱单',
        sending: '出货单',
        order: '订单'
      },
      dialogStatus: ''
    }
  },
  computed: {
  },
  created() {
    // console.log(this.orderId)
  },
  methods: {
    // 上传
    handleUpload(step) {
      this.dialogStatus = step
      this.dialogFormVisible = true
      this.$nextTick(() => {
        console.log(this.orderId)
        // console.log(this.$refs['dataForm'])
        this.$refs['dataForm'].clearValidate()
      })
    },
    updateData() {
      console.log(this.orderNo, this.step, 'uploadData')
      // const methodStep = this.step
      const that = this
      that.formData = new FormData()
      that.$refs['uploade'].submit()
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          that.formData.append('orderNo', this.orderNo)
          that.formData.append('userCode', this.userCode)
          console.log(that.formData, 'formData')
          if (that.formData.get('file').size / 1024 / 1024 >= 10) {
            this.$notify({
              title: '错误',
              message: '上传文件大小不能超过 10MB!',
              type: 'error',
              duration: 3000
            })
            return false
          }
          if (!that.formData.get('file')) {
            this.$notify({
              title: '错误',
              message: '文件不能为空',
              type: 'error',
              duration: 3000
            })
            return false
          }
          if (this.step === 'shipment') {
            shipment(that.formData).then(() => {
              // 直接在页面显示插入的数据
              // this.list.unshift(this.temp)
              this.dialogFormVisible = false
              this.$notify({
                title: '成功',
                message: '创建成功',
                type: 'success',
                duration: 2000
              })
              this.$emit('fetchUploadData', this.orderNo)
            })
          } else if (this.step === 'sending') {
            sending(that.formData).then(() => {
              // 直接在页面显示插入的数据
              // this.list.unshift(this.temp)
              this.dialogFormVisible = false
              this.$notify({
                title: '成功',
                message: '创建成功',
                type: 'success',
                duration: 2000
              })
              this.$emit('fetchUploadData', this.orderNo)
            })
          } else if (this.step === 'order') {
            reUploadOrder(that.formData).then(() => {
              // 直接在页面显示插入的数据
              // this.list.unshift(this.temp)
              this.dialogFormVisible = false
              this.$notify({
                title: '成功',
                message: '再次上传成功',
                type: 'success',
                duration: 2000
              })
              this.$emit('fetchUploadData', this.orderNo)
            })
          }
          that.$refs['uploade'].clearFiles()
        }
      })
    },
    // upload
    handleRemove(file, fileList) {
      console.log(file, fileList)
    },
    handlePreview(file) {
      console.log(file)
    },
    handleExceed(files, fileList) {
      this.$message.warning(`当前限制选择 1 个文件，本次选择了 ${files.length} 个文件，共选择了 ${files.length + fileList.length} 个文件`)
    },
    beforeUpload(file) {
      const testmsg = file.name.substring(file.name.lastIndexOf('.') + 1)
      const extension = testmsg === 'xls'
      const extension2 = testmsg === 'xlsx'
      const extension3 = testmsg === 'xlsm'
      const isLt2M = file.size / 1024 / 1024 < 10
      if (!extension && !extension2) {
        this.$notify({
          title: '错误',
          message: '上传文件只能是 xls、xlsx、xlsm格式!',
          type: 'error',
          duration: 2000
        })
        return false
      }
      if (!isLt2M) {
        this.$message({
          title: '错误',
          message: '上传文件大小不能超过 10MB!',
          type: 'error',
          duration: 2000
        })
        return false
      }
      console.log(extension, extension2, extension3, isLt2M)
      this.formData.append('file', file)
      return (extension || extension2 || extension3) && isLt2M
    }
  }
}
</script>

<style scoped>

</style>
