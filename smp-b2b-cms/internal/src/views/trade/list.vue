<template>
  <div class="user-list">
    <div v-show="resourceCode.indexOf('createOrder') != -1" class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-upload" @click="handleCreate(1)">{{ $t('trade.uploadFutureGoods') }}</el-button>
      <el-button class="filter-item" style="margin-left: 10px;" type="success" icon="el-icon-upload" @click="handleCreate(2)">{{ $t('trade.uploadSaleGoods') }}</el-button>
    </div>
    <div class="filter-container">
      <el-input :placeholder="$t('trade.orderNo')" v-model.trim="listQuery.orderNo" style="width: 240px;" clearable class="filter-item" @keyup.enter.native="handleFilter"/>
      <el-input :placeholder="$t('trade.customerCode')" v-model.trim="listQuery.customerCode" style="width: 200px;" clearable class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <router-link v-show="resourceCode.indexOf('transactionListOptions') != -1" :to="'/trade/edit/'+scope.row.orderNo">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        customerCode: [{ required: true, message: '客户代码不能为空', trigger: 'change' }]
      },
      file: '',
      visibleDel: false
    }
  ...
  created() {
    window._paq.push(['setDocumentTitle', 'B2B交易列表页面'])
  ...
  methods: {
    ...
    async getList() {
      ...
      const str = this.getParams(this.dealObjectValue(this.listQuery))
      try {
        const res = await this.$api.tradeFetchList(str, this.dealObjectValue(this.listQuery))
    ...
    createData() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          console.log('zheli', that.formData)
